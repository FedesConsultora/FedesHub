...
export default function GoogleConnect({ onChanged }) {
  ...
  const { refresh: refreshStatus } = useGoogleStatus()

  const localRef = useRef(null)
  const remoteRef = useRef(null)

  // Disparar una reconexión (cambia de cuenta)
  const handleReconnect = () => {
    connect() // ya incluye el timer que llama reload() al cerrar la ventana
    refreshStatus() // invalidar el caché de estado en React Query
  }
    ...
    <details className="gc-dd">
      ...
      <div className="gc-card">
        ...
        <div className="row">
          ...
          <div className="val">
            <button className="fh-btn tiny ghost" onClick={handleReconnect}>
