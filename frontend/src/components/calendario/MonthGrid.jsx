...
export default function MonthGrid({ year, month, events = [], onDayClick, onEventClick, onQuickAdd }) {
      ...
      <div className="grid">
        {cells.map((d, idx) => {
          ...
          return (
            ...
            >
              <button style={{ padding: '0.5rem', background: 'transparent', border: 'none', color: 'white', fontSize: '1rem' }} className="day-click" onClick={() => onDayClick?.(dateStr)}>
                <span className="num">{d}</span>
              </button>

              <button className="quick-add" onClick={(e) => { e.stopPropagation(); onQuickAdd?.(dateStr); }}>
