...
export default function MentionInput({
  value, onChange, feders = [], disabled = false,
  placeholder = 'Escribir…', className = '', inputRef = null, onPaste
}) {
  ...
  const idx = useMemo(() =>
    feders.map(f => ({
      ...
      fullLower: `${f?.nombre || ''} ${f?.apellido || ''}`.trim().toLowerCase(),
  ...
  const insertToken = (f) => {
    ...
    const display = `@${(f?.nombre||'') + ' ' + (f?.apellido||'')}`.replace(/\s+/g,' ').trim()
  ...
  const onInput = (e) => {
    ...
    if (at >= 0) {
      ...
      if (/^[^\s@]{0,40}$/.test(term)) { setQ(term); setOpen(true); setSel(0); return }
    }
    setOpen(false); setQ(''); setSel(0)
  }
  ...
  const onKeyDown = (e) => {
    ...
    if (e.key === 'Enter' || e.key === 'Tab') { if (results[sel]) { e.preventDefault(); insertToken(results[sel]) } }
        ...
        <div className="mentionPopover" role="listbox" aria-label="Menciones">
          {results.map((f, i) => (
            <button
              ...
              title={f.email || ''}
            >
              <div className="nm">{f.full || `${f.nombre||''} ${f.apellido||''}`}</div>
