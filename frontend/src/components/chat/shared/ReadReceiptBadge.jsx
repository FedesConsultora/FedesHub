...
export default function ReadReceiptBadge({
  canal_id, msg_id, msg_ts, my_user_id, members = [], align='right'
}) {
  if (DEBUG_RRB) {
    // eslint-disable-next-line no-console
    console.debug('RRB props →', { canal_id, msg_id, msg_ts, my_user_id, members })
  }

  const { isDM, totalOtros, vistos, noVistos } = useMemo(() => {
    const otros = (members || []).filter(m => Number(m?.user_id) !== Number(my_user_id))
    ...
    for (const m of otros) {
      ...
      if (seen) v.push(m); else nv.push(m)
    }
    // ordená los vistos por hora (más viejo → más nuevo), luego por nombre
    const byTime = (a,b) => new Date(a?.last_read_at||0) - new Date(b?.last_read_at||0)
  ...
  const hayDatos = [...vistos, ...noVistos].some(m =>
    (m?.last_read_msg_id|0) > 0 || !!m?.last_read_at
  )
  ...
  if (isDM) {
    if (vistos.length === 1) {
      ...
      return (
        <div className={`rrb rrb-dm ${align==='left'?'left':'right'}`} title={at ? `Visto ${at}` : 'Visto'}>
          <i className="ticks ticks-all" aria-hidden>✓✓</i>
...
function ReceiptGroupBadge({ align, allSeen, vistos, noVistos, total }) {
  ...
  return (
    <div className={`rrb rrb-group ${align==='left'?'left':'right'} ${open?'open':''}`}>
      <button
        ...
        type="button"
      >
        <i className={`ticks ${allSeen?'ticks-all':'ticks-part'}`} aria-hidden>✓✓</i>
