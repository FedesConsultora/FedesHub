...
export default function AssignedPeople({
  ...
  const [mode, setMode] = useState('responsable') // responsable | colaborador
  const [search, setSearch] = useState('')

  // IDs ya usados
  const usedIds = new Set([
    ...responsables.map(r => r.id || r.feder_id),
    ...colaboradores.map(c => c.id || c.feder_id)
  ])
  ...
  }, [feders, usedIds, search])

  const handleAdd = (id) => {
    if (mode === 'responsable') onAddResp?.(id)
    else onAddCol?.(id)
    setSearch('')
  }
  ...
  return (
    <div className="peopleGrid">
      ...
      {/* --- LISTAS EXISTENTES --- */}
      <Group title="Colaboradores" items={colaboradores} showRole={true} />

      <Group title="Responsable" items={responsables} showRole={false} />
      ...
      <div className="addSection">
        ...
        <div className="addList">
          {candidates.length === 0 && (
            <div className="empty small">Sin candidatos</div>
          )}
...
// === TU GRUPO ORIGINAL ===
function Group({ title, items = [], showRole = false }) {
  return (
            ...
            return (
              ...
              >
                ...
                <div className="info">
                  ...
                  {showRole && <div className="role">{p.rol || ''}</div>}
                </div>
              </div>
            )
          })
