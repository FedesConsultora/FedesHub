...
export function useSaldos(yearStr) {
  ...
  const fetchAll = () => {
    ...
    Promise.all([
      ...
      .catch(e => setData({ saldos: [], tipos: [], loading: false, error: e?.message || 'Error' }))
  }

  useEffect(() => {
    fetchAll()
    const handlePush = (e) => {
      if (e.detail?.type === 'ausencia' || e.detail?.type === 'ausencia_tipo') fetchAll()
    }
...
export function useAusenciasYear(yearStr, federId = null) {
  ...
  const fetchList = () => {
    ...
    ausenciasApi.aus.list({ feder_id: federId || undefined, desde, hasta })
      ...
      .catch(e => setState({ rows: [], loading: false, error: e?.message || 'Error' }))
  }

  useEffect(() => {
    fetchList()
    const handlePush = (e) => {
      if (e.detail?.type === 'ausencia') fetchList()
    }
    window.addEventListener('fh:push', handlePush)
    return () => window.removeEventListener('fh:push', handlePush)
  }, [yearStr, federId])
