  ...
  return (
    <div className="ClienteDetailPage">
      ...
      <header className="topBar">
        ...
        <h1 className="pageTitle">{cliente.nombre}</h1>
        ...
        <div className="mainCol">
          <section className="infoCard">
              ...
              <div className="clientInfo">
                {!editing ? (
                  ...
                  <div className="editFields">
                    <input type="text" value={form.nombre} onChange={e => upd('nombre', e.target.value)} placeholder="Nombre" className="inputLg" />
                    <input type="text" value={form.alias} onChange={e => upd('alias', e.target.value)} placeholder="Alias (opcional)" />
                  </div>
                )}
              </div>
            </div>
              ...
              {!editing ? (
                ...
                <>
                  ...
                  <select value={form.estado_id} onChange={e => upd('estado_id', Number(e.target.value))}>
                    {(catalog.estados || []).map(e => <option key={e.id} value={e.id}>{e.nombre}</option>)}
                  </select>
                  <select value={form.ponderacion} onChange={e => upd('ponderacion', Number(e.target.value))}>
                    {[1, 2, 3, 4, 5].map(p => <option key={p} value={p}>Pond. {p}</option>)}
                  </select>
            ...
            <div className="contactInfo">
              {!editing ? (
                <>
                  {cliente.email && <div className="contactItem"><FiMail className="icon" /><span>{cliente.email}</span></div>}
                  {cliente.telefono && <div className="contactItem"><FiPhone className="icon" /><span>{cliente.telefono}</span></div>}
                  {cliente.sitio_web && <div className="contactItem"><FiGlobe className="icon" /><a href={cliente.sitio_web} target="_blank" rel="noreferrer">{cliente.sitio_web}</a></div>}
                </>
              ) : (
                <>
                  <div className="inputRow"><FiMail className="icon" /><input type="email" value={form.email} onChange={e => upd('email', e.target.value)} placeholder="Email" /></div>
                  <div className="inputRow"><FiPhone className="icon" /><input type="tel" value={form.telefono} onChange={e => upd('telefono', e.target.value)} placeholder="Teléfono" /></div>
                  <div className="inputRow"><FiGlobe className="icon" /><input type="url" value={form.sitio_web} onChange={e => upd('sitio_web', e.target.value)} placeholder="Sitio web" /></div>
                  <div className="inputRow"><label>Color:</label><input type="color" value={form.color} onChange={e => upd('color', e.target.value)} /></div>
                </>
              )}
            </div>
            ...
            {(cliente.descripcion || editing) && (
              <div className="descSection">
                <h3>Descripción</h3>
                {!editing ? <p>{cliente.descripcion}</p> : <textarea value={form.descripcion} onChange={e => upd('descripcion', e.target.value)} placeholder="Descripción..." rows={3} />}
              </div>
            )}
          </section>
              ...
              <ul className="teamList">
                {cliente.gerentes.map(g => (
                  <li key={g.id} className="teamMember">
                    <div className="memberAvatar" style={{ backgroundImage: g.avatar_url ? `url(${g.avatar_url})` : undefined }}>
              ...
              <ul className="contactList">
                {contactos.map(c => (
                  <li key={c.id} className="contactRow">
                    <div className="contactName">{c.nombre}</div>
                    {c.cargo && <div className="contactCargo">{c.cargo}</div>}
                    {c.email && <a href={`mailto:${c.email}`} className="contactLink">{c.email}</a>}
                  </li>
                ))}
              </ul>
            </section>
