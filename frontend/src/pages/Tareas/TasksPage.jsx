...
export default function TasksPage() {
  ...
  // catálogo (selects)
  const [catalog, setCatalog] = useState({ clientes:[], estados:[], impactos:[], urgencias:[] })
  useEffect(() => { tareasApi.catalog().then(setCatalog).catch(console.error) }, [])
  ...
  useEffect(() => {
    const patch = {}
    const keys = ['q','cliente_id','estado_id','impacto_id','urgencia_id','vencimiento_from','vencimiento_to','orden_by','sort','solo_mias','include_archivadas']
    keys.forEach(k => {
      const v = searchParams.get(k)
      if (v !== null) patch[k] = (k==='solo_mias' || k==='include_archivadas') ? v === 'true' : v
    })
  ...
  useEffect(() => {
    ...
    const urlKeys = ['q','cliente_id','estado_id','impacto_id','urgencia_id','vencimiento_from','vencimiento_to','orden_by','sort','solo_mias','include_archivadas']
    urlKeys.forEach(k => {
      const v = filters[k]
  ...
  const tableRows = useMemo(() => rows.map(t => ({
    id: t.id,
    titulo: t.titulo,
  ...
  return (
    <div className="TareasListPage">
        ...
        <div className="right" style={{ display:'flex', gap:10, alignItems:'center' }}>
          <label className="onlyMine">
            <input
              ...
              onChange={e => setFilters(f => ({ ...f, solo_mias: e.target.checked }))}
            />
            <span>Solo mías</span>
          </label>
      ...
      <section className="filters card">
        <TareasFilters value={filters} catalog={catalog} onChange={setFilters} />
      </section>
      ...
      <section className="results" data-view={view}>
        {view === 'kanban'
          ...
          : <TaskList rows={tableRows} loading={loading} />
        }
      </section>
